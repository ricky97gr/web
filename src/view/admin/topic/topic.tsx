...
const AdminTopicView = () => {
        ...
        },
        {
            ...
            render: (_, { uuid, isShow }) => {
                const onchange = (checked) => {
                    ...
                    myFetch({ url: "/admin/topic", options: { body: body, method: "PUT" } })
        ...
        },
        {
            ...
            render: (_, record) => {
                ...
                const del = () => {
                    ...
                    myFetch({ url: "/admin/topic", options: { body: body, method: "DELETE" } }).then((data) => {
    ...
    const addTopic = (values: any) => {
        console.log(values)
        myFetch({ url: "/admin/topic", options: { body: values, method: "POST" } }).then((data) => {
            if (data.body.code !== 200) {
    ...
    const getAllTopic = () => {
        let param = MyQuery({})
        myFetch({ url: "/admin/topic", options: { method: "GET" }, params: param }).then(
            (data) => {
        ...
        <>
            ...
            <CustomDrawer title="新增话题" isOpen={isshow} UpdateValue={open}>
                <Form onFinish={addTopic}>
                    <Form.Item<NewTopic> label="名称" name="name" rules={[{ required: true, message: "请输入话题名称" }]}>
                        <Input></Input>
                    </Form.Item>
                    <Form.Item<NewTopic> label="启用" name="isShow" valuePropName="checked">
